        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="registerTenantDiv" hidden>
                    ...
                    <div class="container">
                        <div class="container-fluid">
                            <form id="dormManagerRegisterNewTenant" method="POST"
                                action="{{ route('dormManagerRegisterNewTenant') }}" enctype="multipart/form-data">
                                @csrf
                                                        ...
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                ...
                                                                <select name="tenantSex" id="tenantSex"
                                                                    class="form-control">
                                                                    @if (auth()->user()->employee->dormManager->dormitory->sex_accepted == 0)
                                                                        <option value="" selected>Select
                                                                            Gender
                                                                        </option>
                                                                        <option value="0">Female</option>
                                                                    @elseif(auth()->user()->employee->dormManager->dormitory->sex_accepted == 1)
                                ...
                                <div class="page" id="page4" style="display: none;">
                                    <div class="container-fluid">
                                                            ...
                                                            <select name="roomAssignment" id="roomAssignment"
                                                                ...
                                                                @foreach ($dormitoryRooms as $room)
                                                                    @php
                                                                        ...
                                                                        $dormRoomTenantCount = $room->dormRoomStudentTenants
                                                                            ->where('isActive', 1)
                                                                            ->count();
                                                                    @endphp
                                                                    @if ($dormRoomBedCount > $dormRoomTenantCount)
                                        ...
                                        <button type="button" class="btn btn-success float-right"
                                            onclick="showConfirmationModal('new')">Submit</button>
                                        <button type="button"
                                            class="btn btn-default prev-page float-right ">Previous</button>
                                    </div>
                                </div>
                ...
                <div id="showExistingTenant" hidden>
                    ...
                    <form action="{{ route('dormManagerRegisterExistingTenant') }}" method="post"
                        id="dormManagerRegisterExistingTenant">
                        @csrf
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="page" id="page2" style="display: none;">
                                <div class="" id="operatorRegisterExistingTenantError" style="display: none;">
                            ...
                            <div class="page" id="page3" style="display: none;">
                                <div class="container-fluid">
                                    ...
                                    <button type="button" class="btn btn-success float-right"
                                        onclick="showConfirmationModal('existing')">Submit</button>
                                    <button type="button"
                                        class="btn btn-default prev-page float-right ">Previous</button>
...
<script>
    function showConfirmationModal(action) {
        if (action == 'new') {
            $('#confirmationQuestion').text('Confirm submission of new tenant?');

            $('#confirmationModal').modal('show');
            $('#confirmUpdate').click(function() {
                $('#dormManagerRegisterNewTenant').submit();
            });
        } else if (action == 'existing') {
            $('#confirmationQuestion').text('Confirm acceptance of this transferring tenant?');

            $('#confirmationModal').modal('show');
            $('#confirmUpdate').click(function() {
                $('#dormManagerRegisterExistingTenant').submit();
            });
        }
    }
...
<script>
    document.getElementById('roomAssignment').addEventListener('change', function() {
        ...
        $.ajax({
            url: "{{ route('dormManagerFetchRoomsForTenantAssigning') }}",
            type: "POST",
            data: {
                room_id: roomId,
                _token: "{{ csrf_token() }}",
            ...
            success: function(response) {
                ...
                var roomInfo = response[0];

                $('#newTenantDisplayRoomInformation').removeAttr('hidden');
                $('#newTenantRoomName').text(roomInfo.dormRoomName);
                $('#newTenantRoomPrice').text(roomInfo.dormRoomPrice);
                $('#newTenantNumberOfTenants').text(roomInfo.dormRoomTenantCount);
                $('#newTenantNumberOfBeds').text(roomInfo.dormRoomNumberOfBeds);



                if (roomInfo.dormRoomTenants != null && roomInfo.dormRoomTenants.length > 0) {
                    $('#newTenantsRoomTenantsTable tbody').empty();
                    roomInfo.dormRoomTenants.forEach(tenant => {
    ...
    document.getElementById('existingTenantRoomAssignment').addEventListener('change', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#existingTenantRoomName').text(roomInfo.dormRoomName);
                $('#existingTenantRoomPrice').text(roomInfo.dormRoomPrice);
                $('#existingTenantNumberOfTenants').text(roomInfo.dormRoomTenantCount);
                $('#existingTenantNumberOfBeds').text(roomInfo.dormRoomNumberOfBeds);

                if (roomInfo.dormRoomTenants != null && roomInfo.dormRoomTenants.length >
                    0) {
...
<script>
    function verifyStudentId() {
        ...
        $.ajax({
            url: "{{ route('dormManagerFetchStudentTenants') }}",
            type: "POST",
            data: {
            ...
            success: function(response) {
                if (response.status == 'Not Found') {
                    ...
                    if (response.sexId !==
                        {{ auth()->user()->employee->dormManager->dormitory->sex_accepted }} || response
                        .tenantExist == true || response
                        .tenantUncleared == true) {
                        document.getElementById('operatorRegisterExistingTenantError').style.display =
                            'block';
